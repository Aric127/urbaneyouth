...
function sendElasticEmail_old($to, $subject, $body_text, $body_html, $from, $fromName,$attachments)
    ...
    $header = "POST /mailer/send HTTPS/1.0\r\n";
 ...
 $data .= "&attachments=".urlencode($attachments);
   ...
   $biller_id=$this->session->userdata('biller_id');
    ...
    if($billerType=='1')
    {
      ...
      $sql_biller="SELECT `bill_recharge`.bill_pay_date as bill_pay_date,biller_user.bill_invoice_no as bill_invoice_no,bill_recharge.bill_amount as bill_amount,user.user_name as username,user.user_email as useremail FROM `bill_recharge` join biller_user on biller_user.bill_invoice_no=bill_recharge.bill_invoice_no join user on user.user_id=bill_recharge.bill_user_id where bill_recharge.biller_id='".$biller_id."' order by bill_recharge_id DESC limit 5";
	...
	function biller_profile()
  {
        ...
        $this->load->view('biller/menu',$data);
        $this->load->view('biller/header');
        $this->load->view('biller/listing/biller_profile',$data);
        $this->load->view('biller/footer');
  }
  function biller_consumer()
  {
        ...
        $this->load->view('biller/menu',$data);
        $this->load->view('biller/header');
        $this->load->view('biller/listing/biller_consumer',$data1);
        $this->load->view('biller/footer');
  }
...
<html lang="en">
  ...
  <body>
      ...
      <table font-family:Helvetica,Helvetica,Arial,sans-serif; cellpadding="0" cellspacing="0" border="0" width="794px" align="center">
         ...
         <table cellpadding="0" cellspacing="0" border="0" width="100%" align="center" style="background-color:#456776;">
            <tr>
              ...
              <td width="50%"  style="padding: 10px; text-align:left; color: #fff">   '.$title.'
             </td> 
           </tr>
            ...
            <td>
               <table style="padding: 10px">
                    <tr>
                     ...
                     <tr>
                       <td> '.$data[0]->biller_email .'<br> </td> 
                    </tr> 
                     <tr>
                       <td>'.$data[0]->biller_contact_no  .'<br></td> 
                    </tr> 
                    <tr>
             ...
             <td style="text-align: right; vertical-align:top; display: block;">
               ...
               <table style="padding-left:25px; margin-top:10px;">
                    <tr>
                     ...
                     <tr>
                       <td> '.$data[0]->biller_user_email  .'<br></td> 
                    </tr> 
                     <tr>
                       <td> '.$data[0]->biller_user_contact_no  .'<br></td> 
                    </tr> 
               ...
               if(!empty($data[0]->bill_product_id))
    {
      ...
      $total+=($value->biller_invoice_product_price*$value->biller_invoice_product_qty);
               $html .='<tr>
                 <td style="padding:10px; border-right:1px solid #ccc; border-bottom:1px solid #ccc; text-align:left;"> '.$value->biller_invoice_product_name.' </td>
...
<table cellpadding="0" cellspacing="0" border="0" width="794" align="center">
            <tr>
               <td style="background-color:#16262d; padding:15px; text-align:center; color:#fff; font-size:12px; font-style: italic;"> 
      ...
      <table font-family:Helvetica,Helvetica,Arial,sans-serif; cellpadding="0" cellspacing="0" border="0" width="794px" align="center">
         ...
         <table cellpadding="0" cellspacing="0" border="0" width="100%" align="center" style="background-color:#456776;">
            <tr>
              ...
              <td width="50%" align="center" style="padding: 10px; text-align:center; color: #fff">   '.$title.'
             </td> 
           </tr>
       ...
       // $invoice_date=$data[0]->bill_invoice_date;
        ...
        $this->sendElasticEmail('care@oyacharge.com', 'Bill Invoice of '.$invoice_no,"OyaCharge", $message, "care@oyacharge.com", "OyaCharge",$attach['attachId']);
    			...
    			$this->session->set_flashdata('status','Your Invoice INV#'.$invoice_no.' '.'of amount Naira'.' '.$amt.' '.' is generated successfully');
                     ...
                     //  //  $this->email->to($this->input->post("biller_user_email"));
                     ...
                     //    $this->email->send();
                        ...
                        // $this->create_new_pdf_paid($this->input->post("bill_invoice_no")); 
...
// function to add product....--
	function product_list(){
		...
		$biller_id=$this->session->userdata('biller_id');
			...
			$this->load->view('biller/menu',$data);
            $this->load->view('biller/header');
            $this->load->view('biller/listing/product_list_view',$data);
            $this->load->view('biller/footer');
	}
...
function change_status() 
 {
        ...
        else if($table == "church_area" && $function == "church_list"){
           if ($field_value != 1) {
                $message = 'SUCCESS!  Status Change to InActive. ';
            } 

            else {
                $message = 'SUCCESS!  Status Change to Active. ';
            }
            $this->session->set_flashdata('status', $message);
            $path = 'biller/' . $function;
            redirect($path);
     ...
     function add_branch()
   {
	...
	function edit_church() {
        if ($this->input->post('submit')) {
            $data = $this->input->post();
            $where = array('church_area_id' => $this->input->post('church_area_id'));
            unset($data['church_area_id']);
            unset($data['submit']);
           	for($i=0;$i<count($data['church_product_id']); $i++)
		{
			$where12=array('find_in_set(church_product_id, "'.$data['church_product_id'][$i].'") '=>NULL);
			...
			$data['church_product_name']=implode(",",$name);
				...
				$data['church_product_ids']=implode(",",$data['church_product_id']);
		   
	   		$user_image = '';
          unset($data['church_product_id']);
           unset($data['church_product_name']);
			if ($_FILES['church_img']['name']) {
                $user_image = $_FILES['church_img']['name'];
            }
            $attachment = $_FILES['church_img']['name'];
            ...
            if (!empty($attachment)) {
                ...
                if ($new_extension == 'png' || $new_extension == 'jpeg' || $new_extension == 'jpg' || $new_extension == 'bmp') {
				 ...
				 $data['church_area_img'] = $imagename;
            }
           
                unset($data['church_id']);
                unset($data['submit']);
                $this->login_model->update_data('church_area', $data, $where);
	  	  ...
	  	  $where22=array('biller_id'=>$this->session->userdata('biller_id'));
			  $data['biller_details'] = $this->login_model->get_record_join_two_table('biller_details', 'biller_category', 'biller_category_id', 'biller_category_id','*',$where22);
                ...
                $where = array('church_area_id' => $category_id);
               $data['church_details'] = $this->login_model->get_data_where_condition('church_area', $where);
...
function my_bill_sattlement()
{
			...
			$this->load->view('biller/menu',$data);
			$this->load->view('biller/header');
            $this->load->view('biller/listing/sattlement_list',$data);
            $this->load->view('biller/footer');
}
