...
{
	$post = array("status" => "true", "message" => "Version found",'android_version'=>5,'ios_version'=>'1.0');
	echo $this -> json($post);
}
...
function check_secure_pin()
{
				...
				if (!empty($records_user)) {
					...
					if ($status == '1') {
						if ($verify_status == '1') {
							...
							$qr_code = $records_user[0]['qr_code'];
							if(empty($qr_code))
							{
								$this->call_curl($user_id,$user_mobile);
								$QRCode  = BASE_URL."/uploads/QR_Code/".$user_mobile.".png";
							}else{
								$QRCode  = BASE_URL."/uploads/QR_Code/".$qr_code;
							}
							
							$data['user_ip_address'] = $_SERVER['REMOTE_ADDR'];
							$update_toekn = $this -> conn -> updatetablebyid('user', 'user_id', $user_id, $data);
							$post = array("status" => "true", "message" => "Login Successfully", 'user_id' => $user_id, 'user_status' => $status, 'user_name' => $user_name, 'wallet_amount' => $wallet_amount, 'user_profile_pic' => $img, 'user_email' => $user_email, 'user_password' => $u_password, 'mobile' => $user_mobile, 'login_type' => 1, 'user_pin_status' => $pin_status, 'frnd_refferal_code' => $reffer_code,'qr_code'=>$QRCode);
						} else {
				...
				if (!empty($records)) {
					...
					if (!empty($records_user)) {
						...
						if ($status == '1') {
							...
							$data['user_ip_address'] = $_SERVER['REMOTE_ADDR'];
							$update_toekn = $this -> conn -> updatetablebyid('user', 'user_id', $user_id, $data);
								$post = array("status" => "true", "message" => "Login Successfully", 'user_id' => $user_id, 'user_status' => $status, 'user_name' => $user_name, 'wallet_amount' => $wallet_amount, 'profile_pic' => $img, 'user_email' => $user_email, 'user_password' => $u_password, 'mobile' => $user_mobile, 'login_type' => 1, 'user_pin_status' => $pin_status, 'frnd_refferal_code' => $reffer_code,'qr_code'=>$QRCode);
	...
	function social_login() {
		...
		$user_ip_address = $_SERVER['REMOTE_ADDR'];
				...
				if ($insert) {
					$wallet_amount = 0;
					//$QRCode  = $this->call_curl($user_id,$user_contact_no);
					$QRCode = '';
					$post = array("status" => "true", "message" => "Login Successfully", 'user_id' => $insert, 'login_type' => $login_type, 'user_name' => $user_name, 'wallet_amount' => $wallet_amount, 'profile_pic' => $profile_pic, 'user_email' => $user_email, 'user_contact_no' => '', 'verify_status' => '2', 'user_pin_status' => '2', 'frnd_refferal_code' => $reffer_code,'mpin_status'=>'2','qr_code'=>$QRCode);
				}
			}
		} else {
...
//==== verified QR Code via mobile number======//
	function verified_QRCode()
	{
		if(!empty($_REQUEST['mobile_no']))
		{
			$mobile_no 			= $_REQUEST['mobile_no'];
			$records 			= $this -> conn -> get_table_row_byidvalue('user', 'user_contact_no', $mobile_no);
			if(!empty($records))
			{
				$user_id 			= $records[0]['user_id'];
				$user_contact_no 	= $records[0]['user_contact_no'];
				...
				if(empty($user_name))
				{
					$user_name 		= $records[0]['user_email'];
				}
				$user_email 		= $records[0]['user_email'];
				$post = array('status'=>true,'message'=>'QR Code verified Successfully','user_id'=>$user_id,'user_name'=>$user_name,'user_email'=>$user_email,'user_contact_no'=>$user_contact_no,'user_image'=>$img);
			}else{
				$post = array('status'=>false,'message'=>'Invalid QR Code');
			}
			
		}else{
			$post = array('status' => "Failed", "message" => "Missing parameter", 'mobile_no' => $_REQUEST['mobile_no']);
		}
...
function call_curl($user_id,$number)
{
	  			$url = BASE_URL.'/web/genrate_QRCode';
				 $postfields = array('user_id' => $user_id,'user_phone' => $number); 
				 	$ch = curl_init();
	...
	function signup() {
		...
		if (!empty($number) && !empty($user_password)) {
			...
			if (!empty($records)) {
				...
				if ($insert > 0) {
					$this->email_verification($user_email);
					$this->call_curl($insert,$number);
					//, 'token' => $token,
					$post 		= 	array("status" => "true", "message" => "OTP sent to the registered number", 'user_id' => $insert, 'mobile' => $mobile, 'wallet_amount' => $wallet_amount, 'user_email' => $user_email, 'user_password' => $user_password, 'user_status' => 1, 'login_type' => '1', 'self_refferal_code' => $reffer_code, 'frnd_refferal_code' => $frnd_reffer_code,'qr_code'=>BASE_URL."/uploads/QR_Code/".$number.".png");
					echo $this -> json($post);
				}else{
						$post 			= 	array("status" => "false", "message" => "Error in Signup, Please Try agin");
						echo $this -> json($post);
						exit();
				}
			}
...
{
	$path = 'https://'.$_SERVER['HTTP_HOST'].'/email_verify/';
	$path1='https://'.$_SERVER['HTTP_HOST'].'/webassets/images/logo.png';
    ...
    <tbody style=" border-bottom:1px solid #ddd;">
		...
		<div style="padding:20px; background-color: #456776; color:#fff; text-align:center;"><a href=' . $path . "verify_email.php?email=" . base64_encode($user_email) . '>Please click here activate your accout</a></div>
	...
	function wallet_amount() {
		...
		if (!empty($user_id)) {
			...
			if (!empty($records)) {
				...
				if($biller_id!=0)
				{
					$billerrecords = $this -> conn -> get_table_row_byidvalue('biller_details', 'biller_id', $biller_id);
					$biller_name = $billerrecords[0]['biller_name'];
					$biller_type = $billerrecords[0]['biller_type'];
					$biller_logo = biller_company_logo.$billerrecords[0]['biller_company_logo'];
				}else{
						$biller_name = '';
						$biller_logo = '';
						$biller_type = '';
				...
				$post = array("status" => "true", "user_id" => $user_id, 'wallet_amount' => $wallet_amount, 'remaining_sms' => $total_sms, 'total_sms' => $get_sms, 'biller_status' => $biller_status, 'biller_id' => $biller_id,'biller_name'=>$biller_name,'biller_logo'=>$biller_logo,'biller_type'=>$biller_type);
			} else {
	...
	function user_profile() {
		...
		if (!empty($user_id)) {
			...
			if (!empty($records)) {
				...
				$get_sms = $records['0']['get_sms'];
				$total_sms = $records[0]['total_sms'];
				$qr_code = BASE_URL.'/uploads/QR_Code/'.$records[0]['qr_code'];
				if (!empty($profile_pic)) {
				...
				$post = array("status" => "true", "user_id" => $user_id, 'user_name' => $user_name, 'user_email' => $user_email, "user_contact_no" => $user_contact_no, 'user_login_type' => $login_type, 'wallet_amount' => $wallet_amount, 'profile_pic' => $img, 'total_sms' => $get_sms, 'remaining_sms' => $total_sms, 'user_pin_status' => $pin_status, 'reffer_code' => $self_reffer_code,'qr_code'=>$qr_code);
			} else {
...
function plan_category_listing() {
		...
		if(!empty($_REQUEST['mobile_topuptype']))
		{
			$mobile_topuptype = $_REQUEST['mobile_topuptype'];
		}else{
			$mobile_topuptype = '';
		}
		...
		if (!empty($recharge_category)) {
			...
			{
				$records = $this -> conn -> get_table_field_doubles('plan_category', 'plan_recharge_category_id', $recharge_category, 'plan_category_id', 15);
			}else{
			...
			foreach ($records as $v) {
				...
				$plan_category_name = $v['plan_category_name'];
				$arr[] = array('plan_category_id' => $plan_category_id, 'plan_category_name' => $plan_category_name,'mobile_topuptype'=>$mobile_topuptype);
			}
...
for($m=0;$m<count($plan_cat_id);$m++){
	...
	$records_plan = $this -> conn -> join_four_table_new_with_three_field('recharge_plan', 'operator_list', 'recharge_category', 'plan_category', 'recharge_operator_id', 'operator_id', 'recharge_category_id', 'recharge_category_id', 'plan_category_id', 'plan_category_id', 'recharge_plan`.' . '`plan_category_id', $plan_cat_id[$m], 'recharge_operator_id', $operator_id, 'recharge_plan`.' . '`recharge_category_id', $recharge_category, 'rechage_amount');
			...
			foreach ($arr111 as $key => $row22) {
   $date2[$key] = $row22['recharge_amount'];
}
array_multisort($date2, SORT_ASC, $arr111);
				...
				if (empty($wallet_category)) {
					...
					foreach ($records as $values) {
						...
						if ($wt_category == '1') {
							...
							$transaction = $this -> conn -> get_table_field_doubles_order('wallet_transaction', 'wt_category', $wt_category, 'wt_user_id', $user_id, 'wt_id');
						} else if ($wt_category == '4' || $wt_category == '20') {
							//echo "string";
							$transaction = $this -> conn -> get_table_field_doubles_order('wallet_transaction', 'wt_category', $wt_category, 'wt_user_id', $user_id, 'wt_id');
...
//print_r($transaction);
	...
	if($transaction_type ==5 || $transaction_type==3 || $transaction_type==1)
	{
		...
		$operator_image='';
							...
							{
									$imgevent= explode(",",$v['event_image']);
								$operator_image=event_image.$imgevent[0];
							}
							}
...
<table cellpadding="0" cellspacing="0" width="600" style="background:#fff; border:1px solid #cbcbcb; margin:0 auto; font-family:Arial, Helvetica, sans-serif; font-size:12px;">
	...
	*/
	function add_money() {
		...
		$step				=	$_REQUEST['step'];
		$current_date = date("Y-m-d H:i:s");
		$apiFrom 		=   isset($_REQUEST['apiFrom']) && $_REQUEST['apiFrom'] != '' ? $_REQUEST['apiFrom'] : 2;

		if($apiFrom == 1){
			if($card_pay_type=='1')
			  $redirecturl = BASE_URL.'/Payment-Via-Card';
			else
			  $redirecturl = BASE_URL.'/web/payment_via_savecard';
		}else{
			$redirecturl = "";
		}
		...
		if($payment_type == '1' && $step == 1)
		{
			if($card_pay_type=='1')
			{

				$pay	=	$this->payment_card($card_no,$expiry_month,$expiry_year,$cvv_no,$user_id,$user_amount,$savecard_status,$verve_card_status,$verve_card_pin,'1',$apiFrom,$redirecturl);
			}else 
				if($card_pay_type=='2')
				{
					...
					$pay			=	$this->payment_card_token($cardfour_digit,$cardtoken,$cvv_no,$user_id,$user_amount,$cardid,$apiFrom,$redirecturl,$verve_card_status,$verve_card_pin);
				}
		}
		else 
		if($payment_type == '2' && $step == 1)
		{
	...
	//	$pay	=	$this->payment_card($card_no,$expiry_month,$expiry_year,$cvv_no,$user_id,$user_amount);
		...
		if($step == 1){
			...
			$authurl					= $para[4];
		  	...
		  	$para					    = json_decode($_REQUEST['para']);
		  	$status						= $para->status;
	 		$PAYtransaction_id			= $para->trans_id;
	 		$trans_ref_no				= $para->ref_id;
			$data_store_id				= $para->last_id;
		}
		...
		if($status=='error')
		{
			$this->update_transaction($data_store_id, $PAYtransaction_id);
			$post = array('status' => "false", "message" => $PAYtransaction_id,'transaction_date' => date('F-d-Y h:i A',strtotime($current_date)),'transaction_via'=>$transaction_via, 'transaction_id' => $PAYtransaction_id);
			echo $this -> json($post);
		...
		if( $status != 'error'  && $step == 1){

			$post = array('status' => "success", 'step' => 2, 'para' => $para);
			echo $this -> json($post);
			exit();

		}

		if($verve_card_status == 2){
			...
			$transactionStatus  = $_REQUEST['url'];
			$rc 				= "";
			$responseMessage 	= $transactionStatus == 'error' ? 'Sorry, Card Validation was not Successful.' : '';
			$id 				= $trans_ref_no;
			$ref 				= $payment_transaction_id;

		}


		if($transactionStatus == 'fail' OR $transactionStatus == 'error'){
			$this->update_transaction($data_store_id, $responseMessage);
                $current_date = date("Y-m-d H:i:s");
                $wt_type = 0;
                $wt_category = 1;
                $wt_desc = "Add Money";

				$sql="insert into wallet_transaction (wt_user_id,wt_datetime,wt_type,wt_amount,wt_category,transaction_id,wt_desc,trans_ref_no,transaction_data_id,payment_type,wt_status) values ('$user_id','$current_date','$wt_type','$user_amount','$wt_category','$PAYtransaction_id','$wt_desc','$trans_ref_no','$data_store_id','1',3)";
			mysql_query($sql);

			$post = array('status' => "false", "message" => $responseMessage,'rc' => $rc,'transactionStatus'=>$transactionStatus,'id'=>$id,'ref'=>$ref,'transaction_via'=>$transaction_via, 'transaction_ref' => $ref, 'transaction_date' => date('F-d-Y h:i A',strtotime($current_date)), 'transaction_id' => $ref );
	...
	function update_transaction($pay_trans_id = null, $message){
		if($pay_trans_id != null){
			$data['pay_trans_status'] = 'Failed';
			$data['p_response_msg']   = $message;
			// update transaction table
			$update_trans = $this -> conn -> updatetablebyid('payment_transaction', 'pay_trans_id', $pay_trans_id, $data);
		}
	}
	...
	function recharge() {
		...
		if ($wallet_amount >= $recharge_amount) {
			...
			$recharge_status = $this -> mobile_recharge_api($operator_id, $mobile_number, $recharge_amount,$recharge_code,$customer_name,$mobile_topup,$customer_address);
		...
		if ($operator_code == 'MTN') {
			...
			if($recharge_response=='1'){
				...
				if ($recharge) {
					...
					$this->manage_margin($recharge_user_id,$operator_id,$recharge_amount,$recharge_number,$transaction_IDS);
...
//========= manage margin==============
	...
	{
		$this->manage_margin('85','2','100','09770543137');
	}
	function manage_margin($user_id,$operator_id,$recharge_amount,$mobileno,$transID)
	{
			$user_records = $this -> conn -> get_table_row_byidvalue('user', 'user_id', $user_id);
			$is_agent   		= $user_records['0']['is_agent'];
			$wt_amount    		= $user_records['0']['wallet_amount'];
			$user_contact_no    = $user_records['0']['user_contact_no'];
			if($is_agent==1)
			{
			  $margin_rec = $this -> conn -> get_table_field_doubles('agent_users', 'agent_user_id', $user_id, 'agent_service_operator', $operator_id);
			  if(!empty($margin_rec))
			  {
			  	$marign    		 = $margin_rec['0']['agent_margin'];
			  	$agent_status    = $margin_rec['0']['agent_status'];
			  	if($agent_status==1)
			  	{
			  		$cashback_amount = ($marign*$recharge_amount)/100;
				  	$transaction_id  = strtotime("now") . mt_rand(10, 99);
				  	$current_date    = date("Y-m-d H:i:s");
				  	$wallet_type     = 2;//credit
				  	$wallet_category = 21;// margin amount refund
				  	$w_desc          = "Agent marign commission recieved";
					$MArgin_trans    = $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id,wt_datetime,wt_type,wt_amount,wt_category,transaction_id,wt_desc,cashbackrecord_id,trans_ref_no', '"' . $user_id . '","' . $current_date . '","' . $wallet_type . '","' . $cashback_amount . '","' . $wallet_category . '","' . $transaction_id . '","' . $w_desc . '","' . $mobileno . '","'.$transID.'"');
					if($MArgin_trans>0)
					{
						$data['wallet_amount'] = $wt_amount+$cashback_amount;
						$update_wallet 		   = $this -> conn -> updatetablebyid('user', 'user_id', $user_id, $data);
					//$this->send_cashback_message($user_contact_no,$recharge_amount,$cashback_amount);
					}
			  	}
...
<table cellpadding="0" cellspacing="0" width="600" style="background:#fff; border:1px solid #cbcbcb; margin:0 auto; font-family:Arial, Helvetica, sans-serif; font-size:12px;">
	...
	//	$transaction_id = strtotime("now") . mt_rand(10, 99);
		...
		$step			=	$_REQUEST['step'];
		// verve card parameters
		$verve_card_status 	= 	$_REQUEST['verve_card_status'];  // 1 for verve card and 2 for normal
		$verve_card_pin 	= 	$_REQUEST['verve_card_pin'];
		$apiFrom 		=   isset($_REQUEST['apiFrom']) && $_REQUEST['apiFrom'] != '' ? $_REQUEST['apiFrom'] : 2;
		...
		if($payment_type == '1'  && $step == 1)
		{
			if($card_pay_type=='1')
			{
				$pay = $this->payment_card($card_no,$expiry_month,$expiry_year,$cvv_no,$recharge_user_id,$refund_amount,$savecard_status,$verve_card_status,$verve_card_pin,'1',$apiFrom,$redirecturl);
			}else 
				if($card_pay_type=='2')
				{
					...
					$save_card_no	=	$records_cards[0]['save_card_no'];
					$cardfour_digit= substr($save_card_no, -4);
					$pay			=	$this->payment_card_token($cardfour_digit,$cardtoken,$cvv_no,$recharge_user_id,$refund_amount,$cardid,$apiFrom,$redirecturl,$verve_card_status,$verve_card_pin);
		...
		if($step == 1){
			...
			$authurl					= $para[4];
		  	...
		  	$para					    = json_decode($_REQUEST['para']);
		  	$status						= $para->status;
	 		$payment_transaction_id		= $para->trans_id;
	 		$trans_ref_no				= $para->ref_id;
			$data_store_id				= $para->last_id;
		}
		...
		if($status=='error')
		{	
			$this->update_transaction($data_store_id, $payment_transaction_id); 
			$post = array('status' => "false", "message" => $payment_transaction_id,'transaction_date' => date('F-d-Y h:i A',strtotime($current_date)),'transaction_via'=>$transaction_via, 'transaction_id' => $payment_transaction_id);
			echo $this -> json($post);
			exit();
		}
		...
		if($transactionStatus == 'fail' OR $transactionStatus == 'error'){
			...
			$this->update_transaction($data_store_id, $responseMessage); 
			 ...
			 $recharge = $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_type,wt_amount,wt_category,transaction_id,wt_desc,user_wallet_rec_amount,user_card_card_rec_amount,payment_gateway_id,electrice_token_no,payment_type,trans_ref_no,wt_status', '"' . $recharge_user_id . '","' . $current_date . '","' . $wallet_type . '","' . $recharge_amount . '","' . $wt_category . '","' . $transaction_id . '","' . $wt_desc . '","' . $wallet_amount . '","' . $card_deduct_amount . '","' . $payment_transaction_id . '","' . $electricity_token . '","' . $payment_gateway_type . '","' . $trans_ref_no . '"',3);
...
$walletrecharge = $this -> conn -> insertnewrecords('recharge', 'recharge_transaction_id,recharge_user_id,rechage_category,operator_id,rechage_type,recharge_number,recharge_amount,recharge_date,recharge_status,payment_gateway_id,electrice_token_no', '"' . $transaction_id . '","' . $recharge_user_id . '","' . $recharge_category_id . '","' . $operator_id . '","' . $recharge_type_id . '","' . $recharge_number . '","' . $recharge_amount . '","' . $current_date . '","' . $recharge_status . '","' . $payment_transaction_id . '","' . $electricity_token . '"');
	...
	function recharge_from_card() {
		...
		if($payment_type == '1' && $step == 1)
		{
			if($card_pay_type=='1')
			{
				$pay = $this->payment_card($card_no,$expiry_month,$expiry_year,$cvv_no,$recharge_user_id,$recharge_amount,$savecard_status,$verve_card_status,$verve_card_pin,'1',$apiFrom,$redirecturl);

				 // echo 'Stop recharge API'; exit;
				...
				if($card_pay_type=='2')
				{
					...
					$save_card_no	=	$records_cards[0]['save_card_no'];
					$cardfour_digit= substr($save_card_no, -4);
					$pay			=	$this->payment_card_token($cardfour_digit,$cardtoken,$cvv_no,$recharge_user_id,$recharge_amount,$cardid,$apiFrom,$redirecturl,$verve_card_status,$verve_card_pin);
...
//	$pay	=	$this->payment_card($card_no,$expiry_month,$expiry_year,$cvv_no,$recharge_user_id,$refund_amount);
	...
	//	print_r($status); exit;
		...
		if($verve_card_status == 2){
			...
			if($apiFrom == 1)
			  $url =  isset($_REQUEST['url']) && $_REQUEST['url'] != '' ? urldecode($_REQUEST['url']) : '';
			else
			  $url =  isset($_REQUEST['url']) && $_REQUEST['url'] != '' ? $_REQUEST['url'] : '';
		...
		if($transactionStatus == 'fail' OR $transactionStatus == 'error'){
			...
			$recharge = $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_type,wt_amount,wt_category,transaction_id,wt_desc,payment_gateway_id,electrice_token_no,	payment_type,trans_ref_no,transaction_data_id,wt_status', '"' . $recharge_user_id . '","' . $current_date . '","' . $wallet_type . '","' . $recharge_amount . '","' . $wt_category . '","' . $transaction_id . '","' . $wt_desc . '","' . $payment_transaction_id . '","' . $electricity_token . '","' . $payment_gateway_type . '","' . $ref . '","'.$data_store_id.'",3');
		...
		if (!empty($recharge_user_id) && !empty($operator_id) && !empty($recharge_number) && !empty($recharge_amount)) {
			...
			//	if($operator_id=='3'){
		//	$recharge_status = $this -> mobile_recharge_api($operator_id, $mobile_number, $recharge_amount);
		$recharge_status = $this -> mobile_recharge_api($operator_id, $recharge_number, $recharge_amount,$recharge_code,$customer_name,$mobile_topup,$customer_address);
		$biller_records = $this -> conn -> get_table_row_byidvalue('operator_list', 'operator_id', $operator_id);
...
//$post=array('status'=>'true');
				...
				$transaction_id = strtotime("now") . mt_rand(10, 99);
				$trans_id = $transaction_id;
				$recharge = $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_type,wt_amount,wt_category,transaction_id,wt_desc,wt_status,payment_gateway_id,trans_ref_no', '"' . $recharge_user_id . '","' . $current_date . '","' . $wallet_type . '","' . $recharge_amount . '","' . $wt_category . '","' . $payment_transaction_id . '","' . $wt_desc . '","3","' . $payment_transaction_id . '","' . $ref . '"');
...
$this->recharge_mail($recharge_user_id,$operator_id,$recharge_number,$payment_transaction_id,$recharge_amount,$recharge_type,$payment_gateway_type,'2');
				...
				$post = array('status' => "false", "message" => "Recharge failed", "recharge_id" => $recharge, 'recharge_number' => $rec_number, 'recharge_amount' => $recharge_amount, 'wallet_amount' => $wallet_amount, 'transaction_id' => $payment_transaction_id, 'transaction_date' => date('F-d-Y h:i A',strtotime($current_date)),'transaction_ref'=>$ref,'transaction_via'=>$transaction_via);
			}
	...
	function bill_recharge() 
	{
		...
		if (!empty($invoice_no) && !empty($bill_amount) && !empty($recharge_user_id)) {
			...
			if (!empty($bill_records)) {
				...
				$biller_customer_id_no 	= 	$bill_records['0']['biller_customer_id_no'];
				...
				if ($bill_pay_status == '2') {
					...
					if ($wallet_amount >= $bill_amount) {
						...
						if ($recharge) {
							...
							$walletrecharge = $this -> conn -> insertnewrecords('bill_recharge', 'bill_user_id,bill_transaction_id,bill_category_id, biller_id,bill_consumer_no,bill_amount,bill_pay_date,bill_pay_status,bill_invoice_no', '"' . $recharge_user_id . '","' . $transaction_id . '","' . $bill_category_id . '","' . $biller_id . '","' . $biller_customer_id_no . '","' . $bill_amount . '","' . $current_date . '","1","' . $invoice_no . '"');

							// change status of bill///
							if (!empty($walletrecharge)) {

								// $records_biller 			= $this -> conn -> get_table_row_byidvalue('user', 'biller_id', $biller_id);
								// $biller_w_amount 		    = $records_biller['0']['wallet_amount'];
								// $billeruserID 		    	= $records_biller['0']['user_id'];
								// $wt_desc12 ="Add money of pay invoice of bill with invoice no ".$invoice_no;
								// $add_biller_wallet = $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_type,wt_amount,wt_category,transaction_id,wt_desc', '"' . $billeruserID . '","' . $current_date . '","1","' . $bill_amount . '","1","' . $transaction_id . '","' . $wt_desc12 . '"');

								// $dataBiller['wallet_amount']=  $biller_w_amount+$bill_amount;

								// $update_biller_wallet        = $this -> conn -> updatetablebyid('user', 'user_id', $billeruserID, $dataBiller);
								$this->add_biller_wallet_amount($biller_id,$invoice_no,$bill_amount,$transaction_id);
						...
						//echo SITE_URL . "createpdf/pdf/" . $invoice_no."/".$biller_id;die;
...
function add_biller_wallet_amount($biller_id,$invoice_no,$bill_amount,$transaction_id)
{
	$wt_category     = 20;  // add wallet biller to add money
	$current_date    = date("Y-m-d H:i:s");
	$records_biller  = $this -> conn -> get_table_row_byidvalue('user', 'biller_id', $biller_id);
	$biller_w_amount = $records_biller['0']['wallet_amount'];
	$billeruserID    = $records_biller['0']['user_id'];
	$wt_desc12       = "Add money of pay invoice of bill with invoice no ".$invoice_no;
	$add_biller_wallet = $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_type,wt_amount,wt_category,transaction_id,wt_desc', '"' . $billeruserID . '","' . $current_date . '","1","' . $bill_amount . '","'.$wt_category.'","' . $transaction_id . '","' . $wt_desc12 . '"');
	...
	function bill_pay_from_card() {
		...
		$biller_id = $_REQUEST['biller_id'];
		$bill_amount = $_REQUEST['bill_amount'];
		$invoice_no = $_REQUEST['bill_consumer_no'];
		$wallet_type = 2;
		// 1- Credit, 2-Debit
		$bill_pay_status = 1;
		$wallet_category = '4';
	...
	if (!empty($bill_records)) 
	{
		...
		if($payment_type == '1'  && $step == 1)
		{
			if($card_pay_type=='1')
			{
				$pay = $this->payment_card($card_no,$expiry_month,$expiry_year,$cvv_no,$recharge_user_id,$bill_amount,$savecard_status,$verve_card_status,$verve_card_pin,'1',$apiFrom,$redirecturl);
			}else 
				if($card_pay_type=='2')
				{
					$records_cards = $this -> conn -> get_table_field_doubles('save_card_details', 'save_card_userid', $recharge_user_id, 'save_card_token', $card_token);
					...
					$save_card_no	=	$records_cards[0]['save_card_no'];
					$cardfour_digit = substr($save_card_no, -4);
					$pay			=	$this->payment_card_token($cardfour_digit,$cardtoken,$cvv_no,$recharge_user_id,$bill_amount,$cardid,$apiFrom,$redirecturl,$verve_card_status,$verve_card_pin);
				}
		}
		else 
		if($payment_type == '2'  && $step == 1)
		...
		if($transactionStatus == 'fail' OR $transactionStatus == 'error'){
			...
			$this->update_transaction($data_store_id, $responseMessage);
			  ...
			  $recharge = $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_type,wt_amount,wt_category,transaction_id,wt_desc,wt_status,trans_ref_no,payment_gateway_id,payment_type', '"' . $recharge_user_id . '","' . $current_date . '","' . $wallet_type . '","' . $bill_amount . '","' . $wt_category . '","' . $payment_transaction_id . '","' . $wt_desc . '",3,"' . $trans_ref_no . '","' . $payment_transaction_id . '","' . $payment_gateway_type . '"');
				...
				if ($bill_pay_status == '2') {
					...
					if ($recharge) {
						...
						if (!empty($walletrecharge)) {
							...
							$this->add_biller_wallet_amount($biller_id,$invoice_no,$bill_amount,$payment_transaction_id);
							$data_frnd['bill_paid_date'] = $current_date;
							$data_frnd['bill_pay_status'] = 1;
							$update_toekn = $this -> conn -> updatetablebyid('biller_user', 'biller_user_id', $bill_user_id, $data_frnd);
					   ...
					   $this->send_bill_user_msg($user_number,$res,$bill_consumer_no,$bill_amount);
						...
						// send mail to user
						$this->send_bill_user_mail($user_email,$invoice_no,$bill_consumer_no,$bill_amount);


$curl = curl_init();
$paid_pdf_url=basr_url('createpdf/create_new_pdf_paid');
curl_setopt_array($curl, array(
  CURLOPT_URL => $paid_pdf_url,
...
if ($err) {
 // echo "cURL Error #:" . $err;
  ...
  //echo $response;
						...
						$post = array("status" => "true", 'message' => "Bill Paid Successfully", "bill_recharge_id" => $recharge, 'consumer_no' => $bill_consumer_no,'bill_invoice_no'=>$invoice_no, 'bill_amount' => $bill_amount, 'wallet_amount' => $wallet_amount, 'transaction_id' => $payment_transaction_id,'transaction_date' => date('F-d-Y h:i A',strtotime($current_date)),'transaction_ref'=>$trans_ref_no,'transaction_via'=>$transaction_via);
					} else {
	...
	if (!empty($bill_records)) 
	{
			...
			if($card_pay_type=='1')
			{
				...
				if($card_pay_type   == '2')
				{
					...
					$pay			=	$this->payment_card_token($cardfour_digit,$cardtoken,$cvv_no,$recharge_user_id,$bill_amount,
						$cardid,$apiFrom,$redirecturl,$verve_card_status,$verve_card_pin);
				}
		}
		else 
		if($payment_type == '2'  && $step == 1)
		...
		if($transactionStatus == 'fail' OR $transactionStatus == 'error'){
			...
			$this->update_transaction($data_store_id, $responseMessage);
			 ...
			 $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_type,wt_amount,wt_category,transaction_id,wt_desc,user_wallet_rec_amount,user_card_card_rec_amount,trans_ref_no,	wt_status', '"' . $recharge_user_id . '","' . $current_date . '","' . $wallet_type . '","' . $bill_amount . '","' . $wt_category . '","' . $payment_transaction_id . '","' . $wt_desc . '","' . $wallet_amount . '","' . $card_deduct_amount . '","' . $trans_ref_no . '",3');
		...
		if (!empty($company_name) && ($company_reg_no)  && !empty($bussiness_phone) && !empty($biller_name) && !empty($biller_name) && !empty($_FILES['bussiness_logo']['name']) && !empty($user_id)) {
			...
			if (empty($biller_records)) {
				...
				if (!empty($attachment)) {
					...
					$user_records = $this -> conn -> get_table_row_byidvalue('user', 'user_id', $user_id);
					$pass = $user_records[0]['user_password'];
					$original_pass 	= '';
				//	$biller_pass 	= md5($original_pass);
					$biller_insert 	= $this -> conn -> insertnewrecords('biller_details', 'biller_category_id,biller_name,biller_email,company_reg_no,rc_no,tin_no,biller_address,biller_contact_no,biller_company_name,biller_company_logo,biller_created_date,biller_status,biller_reg_type,biller_password,biller_original_pass,email_verify_status', '"' . $biller_category_id . '","' . $biller_name . '","' . $biller_email . '","' . $company_reg_no . '","' . $rc_no . '","' . $tin_no . '","' . $bussiness_address . '","' . $bussiness_phone . '","' . $company_name . '","' . $biller_company_logo . '","' . $current_date . '","' . $biller_status . '","' . $biller_reg_type . '","' . $pass . '","' . $original_pass . '","1"');
...
function remove_offer_cart()
{
	...
	function recharge_dstv()
	{
		...
		$moble_topup=$_REQUEST['moble_topup'];
		$this->mobile_recharge_api($operator_id,$mobile,$amount,$recharge_code,$customer_name,$moble_topup);
	}
	function mobile_recharge_api($operator_id, $mobile, $amount,$recharge_code,$customer_name,$moble_topup,$customer_address) {
		...
		if($moble_topup==2)
		{
			$operator_code = $biller_records['0']['operator_bundle_code'];
		}
		if($operator_id==16)
		{
			$plan_records = $this -> conn -> get_table_field_triple_rechargeplan('recharge_plan', 'recharge_operator_id', $operator_id,'rechage_amount',$amount,'ersPlanId','');
			$ersPlanId = $plan_records[0]['ersPlanId'];
		}else{
			$ersPlanId ='';
		}
		//|| $operator_code == 'ALC'
		if ($operator_code == 'MTN' || $operator_code == 'VISAF' ) 
	{
     ...
     // corncone api for recharge==================//////////////////==========
		else if($operator_code == 'AQA' || $operator_code == 'AWA' || $operator_code == 'AQC' || $operator_code == 'ANA'||  $operator_code == 'ANB'||  $operator_code == 'APA' || $operator_code == 'AEA' || $operator_code == 'ACA' || $operator_code == 'APB'  || $operator_code == "ADA"|| $operator_code == "AUB"|| $operator_code == "AEC"|| $operator_code == "ACC"|| $operator_code == "ADC" || $operator_code == "ALC")
		{
			$phone=$mobile1;
			$id=rand('1000','9999');
			$service_id=$operator_code;
			$amt=$amount;
			if($operator_code == 'ACA'||$operator_code == 'ADA'||$operator_code == 'AEC' || $operator_code=='ALC')
			{
			...
			if($operator_code == 'ADC')
			{
			
				$arr = array(
					   'details' => array('phoneNumber' => $phone, 'amount' => $amt,'ersPlanId'=>$ersPlanId),
					   'id' => $id,
					   'paymentCollectorId' => 'CDL',
					   'paymentMethod' => 'PREPAID',
					   'serviceId' => $service_id
			...
			if($operator_code == 'APA')
			{
				...
				$arr = array(
					   'details' => array('accountNumber' => $phone, 'amount' => $amt,'phoneNumber'=>'09770112233','email'=>'abc@gmail.com','customerName'=>$customer_name,'customerAddress'=>$customer_address,'contactType'=>'LANDLORD'),
					   'id' => $id,
					   'paymentCollectorId' => 'CDL',
					   'paymentMethod' => 'PREPAID',
					   'serviceId' => $service_id
			...
			if($operator_code == 'APB')
						{
							...
							$arr = array(
								   'details' => array('meterNumber' => $phone, 'amount' => $amt,'phoneNumber'=>'09770112233','email'=>'abc@gmail.com','customerName'=>$customer_name,'customerAddress'=>$customer_address,'contactType'=>'LANDLORD'),
								   'id' => $id,
								   'paymentCollectorId' => 'CDL',
								   'paymentMethod' => 'PREPAID',
								   'serviceId' => $service_id
	...
	$arr = array('details' => array("requestType"=>"FIND_STANDALONE_PRODUCTS"),
			...
			array(
				...
				for($i=0;$i<count($new_data)-1;$i++){
									$arr121 = array('details' => array("primaryProductCode"=>$new_data[$i]->code,"requestType"=>"FIND_PRODUCT_ADDONS"),'serviceId' => $service_id);
					...
					if ($err) {
						...
						$arr=json_decode($response);
						 ...
						 if(!empty($firstName))
						 {
						 	...
						 	if($service_id=='ANB' || $service_id=='ANA' || $service_id=='BGA')
								{
										
									$arr = array('details' => array("customerAccountId"=>$data_number,"requestType"=>"GET_BUNDLES"),'serviceId' => $service_id);
							$requestBody = json_encode($arr);	
							$hashedRequestBody = base64_encode(hash('sha256', utf8_encode($requestBody), true));
							$date = gmdate('D, d M Y H:i:s T');
							$signedData = "POST" . "\n" . $hashedRequestBody . "\n" . $date . "\n" . "/rest/consumer/v2/exchange/proxy";
				...
				"x-msp-date:" . $date
					...
					//	print_r($arr);
						...
						}else if($status=='2')
						{
						 ...
						 {
						 	$address = $arr->details->address;
						 echo	$firstName=$arr->details->name.",".$address;
...
//$URL="https://baxi.capricorndigi.com/app/rest/consumer/v2/exchange/proxy"; // live
$URL="https://test.platform.baxibox.com/app/rest/consumer/v2/exchange/proxy"; // test
					$curl = curl_init();
					curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, 0);
					curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, 0);
					...
					if ($err) {
						...
						$arr=json_decode($response); print_r($arr);
					}
}
				...
				"x-msp-date:" . $date
					...
					if ($err) {
						...
						if($status=='0')
						{
							...
							$lastName=$arr->details->lastName;
							$address=$arr->address;
							 $post = array('status' => "true", "customer_name" => $firstName." ".$lastName,'address'=>$address);
...
Payment');
						 }else if($status!='0' && $status!='1' && $status!='2' && empty($errorMessage))
						 {
						 	...
						 	$address=$arr->address;
							 $post = array('status' => "true", "customer_name" => $firstName,'address'=>$address);
	...
	function send_cashback_message($mobile_no,$recharge_amount,$cashback_amount)
	{
		$user_name=sms_user_name;
		$password=sms_password;
		$mobile =  $mobile_no;
		echo $msg= "Thanks for the transaction. You received cashback of " . $cashback_amount . " NGN via OyaCharge Agent program with transaction amount NGN " . $recharge_amount . ". For more services visit www.oyacharge.com Happy to Help !!!  care@oyacharge.com";
		$encodedMessage = urlencode($msg);
			$url = "http://www.kudisms.net/components/com_spc/smsapi.php?username=$user_name&password=$password&sender=OyaCharge&recipient=$mobile&message=" .$encodedMessage;
		$ch = curl_init();
		curl_setopt_array($ch, array(CURLOPT_URL => $url, CURLOPT_RETURNTRANSFER => true, CURLOPT_POST => true, CURLOPT_FOLLOWLOCATION => true));
			...
			if (!empty($bill_records)) {
				$church_biller_id = $bill_records['0']['church_biller_id'];
					...
					if ($wallet_amount >= $church_product_price) {
						...
						$this->biller_wallet_upgrade($church_biller_id,$church_product_price);
...
function donate_church_with_card()
{
	if(!empty($_REQUEST['church_id'])&&($_REQUEST['donar_user_id'])&&($_REQUEST['church_category_id'])&&($_REQUEST['church_product_id'])&&($_REQUEST['church_product_price']) &&($_REQUEST['church_area_id']))
	{
		...
		if($payment_type == '1' && $step == 1)
		{
			...
			if($card_pay_type=='1')
			{
				$pay = $this->payment_card($card_no,$expiry_month,$expiry_year,$cvv_no,$donar_user_id,$church_product_price,$savecard_status,$verve_card_status,$verve_card_pin,'1',$apiFrom,$redirecturl);
			}else 
				if($card_pay_type=='2')
				{
					...
					$save_card_no	=	$records_cards[0]['save_card_no'];
					$cardfour_digit	= substr($save_card_no, -4);
					$pay			=	$this->payment_card_token($cardfour_digit,$cardtoken,$cvv_no,$donar_user_id,$church_product_price,$cardid,$apiFrom,$redirecturl,$verve_card_status,$verve_card_pin);
				}
		}
		else 
		if($payment_type == '2' && $step == 1)
		...
		if($transactionStatus == 'fail' OR $transactionStatus == 'error'){
			...
			$recharge 	= $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_category,wt_amount,transaction_id,wt_desc,trans_ref_no,payment_gateway_id,payment_type,wt_status', '"' . $donar_user_id . '","' . $current_date . '","' . $wt_category . '","' . $church_product_price . '","' . $payment_transaction_id . '","' . $wt_desc . '","' . $trans_ref_no . '","' . $payment_transaction_id . '","' . $payment_type . '",3');
...
function donate_church_wallet_with_card()
{
	if(!empty($_REQUEST['church_id'])&&($_REQUEST['donar_user_id'])&&($_REQUEST['church_category_id'])&&($_REQUEST['church_product_id'])&&($_REQUEST['church_product_price'])&& $_REQUEST['payment_gateway_price'])
	{
		...
		if($transactionStatus == 'fail' OR $transactionStatus == 'error'){
			...
			$this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_amount,wt_category,transaction_id,wt_desc,user_wallet_rec_amount,user_card_card_rec_amount,trans_ref_no,wt_status', '"' . $donar_user_id . '","' . $current_date . '","' . $church_product_price . '","' . $wt_category . '","' . $payment_transaction_id . '","' . $wt_desc . '","' . $wallet_amt . '","' . $card_rec_amt . '","' . $trans_ref_no . '",3');
...
function mail_send()
{
	...
	$recharge_amount=$_REQUEST['amount'];
	 ...
	 $this->recharge_mail($recharge_user_id,$operator_id,$recharge_number,$transaction_id,$recharge_amount,$rechage_type,$pay_type,$status);
}
...
<body>
<table style="width:100%; max-width:600px; margin:0 auto;background-color:#456776" cellspacing="0" cellpadding="0">
  <tbody>
    


     <tr>
      <td width=""><table width="" cellspacing="0" cellpadding="0" align=""style="width:100%;">
          <tbody>
            <tr>
              <td colspan="2" style="text-align:left;padding-top:15px;padding-left:20px"><img src="https://oyacharge.com/assets/images/logo_1.png" alt="Oyacharge" title="Oyacharge" class="CToWUd" /></td>
              <td colspan="2" style="text-align: right;padding-top: 15px;padding-left: 20px;color: #fff;padding: 30px;">
                '.$current_date.'
              </td>
    ...
    <tr>
     <td style="text-align:center; padding-top:20px;font-family:Arial,Helvetica,sans-serif;color: #FFEB3B;font-size: 30px;font-weight: bold;">
        Payment Successful!
      </td>
    ...
    <tr align="center">
      <td style="font-size:14px;font-family:Arial,Helvetica,sans-serif;color:#fff"><p><span style="color: #FFEB3B;font-weight: bold;">Dear '.$user_name.',</span> thanks for transacting with us.</p>
      ...
      <td><table style="background-color:#fff;border-width:1px;border-color:#d1d1d1;border-top-color:#57a9a0;border-style:solid;border-top-width:2px" align="center">
          <tbody>
            <tr>
              ...
              <td style="font-size:15px;font-family:Arial,Helvetica,sans-serif;color:#6b6a6b;border-right:1px solid #d1d1d1" width="190">'.$recharge_no;
              $message.='<p style="margin:0;font-size:9px;font-family:Arial,Helvetica,sans-serif;color:#a7a7a7;text-transform:uppercase;margin-top:5px"> '.$operator_name.' </p></td>
              <td style="padding-left:10px" width="54"></td>
              <td style="font-size:15px;font-family:Arial,Helvetica,sans-serif;color:#6b6a6b" width="190"><p><span><img src="'.$currency_icon.'" /></span>'.$recharge_amount.'</p></td>
            </tr>
            <tr>
              <td colspan="4" style="text-transform:uppercase;font-size:9px;font-family:Arial,Helvetica,sans-serif;color:#a7a7a7;border-top:1px solid #d1d1d1;padding:15px 20px"> ORDER ID: <span style="font-size:13px;color:#6b6a6b">'.$trans_id.'</span></td>
...
function test_sms_api()
{
	...
	//	$code = 1234;
		$mobile =  $mobile_no;
		// $mobileNumber = substr($mobile_no, 1);
		$msg="Your OyaCharge Account activation OTP is : $code, use this secure OTP to register with OyaCharge.";
		//$msg="Your OyaCharge Account activation OTP is : $code";
	//	$msg = "Recharge Verification Code : " . $code;
		$encodedMessage = urlencode($msg);
		//$encodedMessage = $msg;
		  $url = "http://www.kudisms.net/components/com_spc/smsapi.php?username=$user_name&password=$password&sender=OyaCharge&recipient=$mobile&message=" .$encodedMessage;
...
//invoice email to payer user
function send_bill_user_mail($email,$invoice_no,$consumer_no,$amount)
{
	
				$data='Your Invoice INV#'.$invoice_no.' '.'of amount Naira'.' '.$amount.' '.' is paid of consumer no.'.' '.$consumer_no;
			    $user_email = $email;
				$subject = 'Pay';
				$message= '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml"><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /><title>Untitled Document</title></head>
...
<table cellpadding="0" cellspacing="0" width="600" style="background:#fff; border:1px solid #cbcbcb; margin:0 auto; font-family:Arial, Helvetica, sans-serif; font-size:12px;">
    ...
    <tbody style=" background:#FEFEFE; border-bottom:1px solid #ddd;">
    	<tr>
        	<td style="padding:10px 15px;">
            	<h1 style="margin-bottom:0px; color:#337d75;">OyaCharge </h1>
            	<p> Dear User, below is your payment detail for invoice. 
</p>';
            $message .=  '<p>'.$data.'</p>';
...
if(!empty($true_record) && empty($false_record))
{
				...
				$records = $this -> conn -> get_table_row_byidvalue('user', 'user_id', $user_id);
					...
					if (!empty($coupon_id)) {
						...
						$this->biller_wallet_upgrade($biller_id,$ticket_price);
...
if(!empty($true_record) && empty($false_record))
{
		...
		if($payment_type == '1'  && $step == 1)
		{
			if($card_pay_type=='1')
			{
				$pay = $this->payment_card($card_no,$expiry_month,$expiry_year,$cvv_no,$user_id,$payment_gateway_price,$savecard_status,$verve_card_status,$verve_card_pin,'1',$apiFrom,$redirecturl);
			}else 
				if($card_pay_type=='2')
				{
					$records_cards = $this -> conn -> get_table_field_doubles('save_card_details', 'save_card_userid', $user_id, 'save_card_token', $card_token);
					...
					$save_card_no	=	$records_cards[0]['save_card_no'];
					$cardfour_digit = substr($save_card_no, -4);
					$pay			=	$this->payment_card_token($cardfour_digit,$cardtoken,$cvv_no,$user_id,$payment_gateway_price,$cardid,$apiFrom,$redirecturl,$verve_card_status,$verve_card_pin);
				}
		}
		else 
		if($payment_type == '2'  && $step == 1)
		...
		if($transactionStatus == 'fail' OR $transactionStatus == 'error'){
			...
			$this->update_transaction($data_store_id, $responseMessage);
               $ticket_price 		  =	$_REQUEST['ticket_price'];
				...
				$wt_category 		  = '16';
				$wt_desc 			  =	"Event Ticket Booking";
				$payment_gateway_type =	$payment_type; 
				$transaction_id  = strtotime("now") . mt_rand(10, 99);
			$this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_category,wt_amount,transaction_id,wt_desc,payment_type,payment_gateway_id,user_card_card_rec_amount,trans_ref_no,wt_status', '"' . $user_id . '","' . $current_date . '","' . $wt_category . '","' . $ticket_price . '","' . $transaction_id . '","' . $wt_desc . '","' . $payment_gateway_type . '","' . $payment_gateway_id . '","' . $payment_gateway_price . '","' . $trans_ref_no . '",3');
					...
					$data['wallet_amount'] = $user_wallet;
					   ...
					   $update_toekn = $this -> conn -> updatetablebyid('user', 'user_id', $user_id, $data);
						...
						$this->biller_wallet_upgrade($biller_id,$ticket_price);
							...
							echo $this -> json(array('status' => 'error'));
						}
	...
	if(!empty($true_record) && empty($false_record))
	{
		...
		if($transactionStatus == 'fail' OR $transactionStatus == 'error'){
			...
			$this->update_transaction($data_store_id, $responseMessage);
			 ...
			 $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_category,wt_amount,transaction_id,wt_desc,payment_type,payment_gateway_id,user_wallet_rec_amount,user_card_card_rec_amount,trans_ref_no,wt_status', '"' . $user_id . '","' . $current_date . '","' . $wt_category . '","' . $ticket_price . '","' . $transaction_id . '","' . $wt_desc . '","' . $payment_gateway_type . '","' . $payment_gateway_id . '","' . $wallet_amt . '","' . $card_rec_amt . '","' . $trans_ref_no . '",3');
  ...
  <table align="center" border="0" cellpadding="0" cellspacing="0" height="100%" width="100%">
    <tr>
      <td align="center" valign="top" width="100%" class="background">
        <center>
          <table cellpadding="0" cellspacing="0" width="600" style="box-shadow: 0 1px 4px 0 rgba(0, 0, 0, 0.14);margin-top: 50px;background-color: #fff;padding: 40px">

            <tr style="width: 100%">
              <td valign="top" class="wrap-cell" style="width: 40%;text-align: center;    border: 1px solid #ddd;">
                
                
    			  <span style="color: #43a047;display: block; padding: 5px 0;font-size: 14px;font-weight: bold;margin-top: 50px">Order ID: #'.$transaction_idss.'</span>
                <span style="color: #333;display: block;padding: 5px 0;font-size: 14px;font-weight: bold;">Order For</span>
                <span style="color: #333;display: block;padding: 5px 0;font-size: 14px;">'.$event_name.'</span>
                <span style="color: #333;display: block;padding: 5px 0;font-size: 14px;">'.$event_date.'</span>
              </td>
              <td style="width: 10%;border: 1px solid #ddd;"></td>
              <td style="width: 50%;padding-left: 25px;border: 1px solid #ddd;">
              <span style="background-color: #fff;display: block;padding: 20px 0;
    border: 1px dotted #ddd;text-align: center;"><img  style="padding: 10px;width: 170px" src="'.$event_qrcode.'"><strong style="color: #a06002; ">Ordered Date: '.$transaction_date.'</strong> </span>
              </td>
            </tr>

            <tr style="width: 100%;padding: 10px 0;">
              <td style="width: 45%;padding:20px 0;">
                <span style="color: #000;display: block;font-weight: bold;">Order By</span>
                <span style="color: #333;display: block;padding: 3px 0;font-size: 14px;">'.$user_name.'</span>
                <span style="color: #333;display: block;padding: 3px 0;font-size: 14px;">'.$user_email.'</span>
               
              </td>
              <td style="width: 10%;padding:20px 0;"></td>
              <td style="width: 45%;padding:20px 0;">
                <span style="color: #000;display: block;font-weight: bold;">Event Venue</span>
                <span style="color: #333;display: block;padding: 3px 0;font-size: 14px;">'.$event_place.' </span>
                <span style="color: #333;display: block;padding: 3px 0;font-size: 14px;">'.$event_contact_no.'</span>
               
              </td>
            </tr>
            <tr>
              <td colspan="3">
                <strong style="margin-bottom: 5px;">Order details</strong>
                ...
                <table style="width:100%">
                  ...
                  <tr style="text-align: center;">
                    ...
                    <td style="width:20%;font-weight: bold;color: #fff;font-size: 12px;">Ticket Name</td>
                    <td style="width:20%;font-weight: bold;color: #fff;font-size: 12px;">Ticket Qty.</td>
                    <td style="width:20%;font-weight: bold;color: #fff;font-size: 12px;">Price</td>
                    <td style="width:20%;font-weight: bold;color: #fff;font-size: 12px;">Subtotal</td>
                ...
                while($row=mysql_fetch_array($response)){
                 ...
                 <table style="width: 100%;">
                  <tr style="width:100%;">
                    ...
                    <td style="width: 50%;padding: 40px 0;">
            ...
            <tr>
              <td colspan="3" style="font-size: 12px;background-color: #43a047;padding: 15px;text-align: center;color: #fff;">
            ...
            <tr>
                ...
                <span>Powered by <img width="70px" src="https://oyacharge.com/wassets/images/logo.png"></span>
              </td>
              <td style="font-size: 12px;background-color: #343a40;padding: 15px;text-align: center;color: #fff;"></td>
              <td style="font-size: 12px;background-color: #343a40;padding: 15px;text-align: center;color: #fff; ">Address: 8B, Lalupon close, Ikoyi Lagos</td>
...
function payment_card($card_no,$expiry_month,$expiry_year,$cvv_no,$user_id,$amount,$save_card_status,$verve_card_status,$verve_card_pin,$userType,$apiFrom='',$redirecturl="")
{
...
//	$apikey			=	$_REQUEST['api_key'];
  ...
  curl_setopt_array($curl, array(
  CURLOPT_URL => "https://live.moneywaveapi.co/v1/transfer",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 300,
...
if ($err) {
 // echo "cURL Error #:" . $err;
  ...
  if($status == 'success')
  {
  	 ...
  	 $chargeMethod 		=  $data->data->chargeMethod;
	...
	if($save_card_status=='1')
			{
				...
				// $token=$this->create_token_moneywave();
				 $save_card=$this->save_card_details($card_no,$expiry_month,$expiry_year,$cvv_no,$token,$user_id,$cardid,$verve_card_status);
				
			}
		//return $status."/".$trans_id."/".$ref_id."/".$last_id."/".$authurl;
		return array( 'status' => $status, 'trans_id' => $trans_id, 'ref_id' => $ref_id, 'last_id' => $last_id, 'authurl' => $authurl, 'response_msg' => $response_msg, 'chargeMethod' => $chargeMethod );
	  
  }else{
  		return array('status' => $data->status, 'message' => $msg);
...
function payment_card_token($cardlast_four,$cardtoken,$cvv_no,$user_id,$amount,$cardid,$apiFrom,$redirecturl="",$verve_card_status,$verve_card_pin)
{
  ...
  if($status == 'success')
  {
	...
	// $insert = $this -> conn -> insertnewrecords('payment_transaction','pay_trans_userid,p_transaction_id,p_reffernce_id, pay_trans_amount,trans_pay_type,p_response_msg,pay_trans_data,pay_trans_status,p_trans_datetime', '"' . $user_id . '","' . $trans_id . '","' . $ref_id . '","' . $trans_amount . '","' . $payment_type . '","' . $respoonse_msg . '", "'".$data1212."'" ,"'.$status.'","' . $current_date . '"');
  	...
  	//return  $data->status."/".$msg;
			...
			return array('status' => 'error', 'message' => 'Token not created!');
		}
	}else{
		return array('status' => 'error', 'message' => 'Parameter missing2!');
	}
	...
	// $trans_id		=	$record->id;
	 ...
	 $im= mysql_query($sql);
  	...
  	//return  $data->status."/".$msg;
			...
			return array('status' => 'error', 'message' => "Paramtere missing!");
		}
	}else{
			return array('status' => 'error', 'message' => "Paramtere missing!");
			exit();
...
curl_setopt_array($curl, array(
  ...
  CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
  CURLOPT_CUSTOMREQUEST => "POST",
  CURLOPT_POSTFIELDS => "{\"apiKey\":\"lv_OK474IQ075CH8V35XB6H\",\n\"secret\":\"lv_VMRVV9HFLYIXDMG2Z3H3FG43RJ2ZBM\"}",
  CURLOPT_HTTPHEADER => array(
    "cache-control: no-cache",
    "content-type: application/json",
    "postman-token: 9a30a5fd-69af-7227-f8cc-29366cd776f0"
  ),
));
...
function chargeAuth()
{
	...
	$transactionRef		=	$_REQUEST['transactionRef'];
	$otp				=	$_REQUEST['otp'];
	...
	curl_setopt_array($curl, array(
	  CURLOPT_URL => "https://live.moneywaveapi.co/v1/transfer/charge/auth/card",
	  CURLOPT_RETURNTRANSFER => true,
	  CURLOPT_ENCODING => "",
	  CURLOPT_MAXREDIRS => 10,
	  CURLOPT_TIMEOUT => 30,
	  CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
	  CURLOPT_CUSTOMREQUEST => "POST",
	  CURLOPT_POSTFIELDS => "{ \"transactionRef\": \"$transactionRef\", \"otp\": \"$otp\"}",
	  CURLOPT_HTTPHEADER => array(
	    "cache-control: no-cache",
	    "content-type: application/json",
	    "postman-token: ed8e8fb9-a31d-124c-eccc-732e952ee1d1"
	  ),
	));
	...
	if ($err) {
	  ...
	  if($status=='success')
	  {
	  	$post = array('status'=>'success','message'=>$result->message,'data'=>$result->data);
	  }else{
	  	$post = array('status'=>'error','message'=>$result->message,'data'=>$result->data);
	  }
	}
...
function AcoountOtpVerify()
{
	...
	$curl = curl_init();
	 ...
	 curl_setopt_array($curl, array(
	  CURLOPT_URL => "https://live.moneywaveapi.co/v1/transfer/charge/auth/account",
	  CURLOPT_RETURNTRANSFER => true,
	  CURLOPT_ENCODING => "",
	  CURLOPT_MAXREDIRS => 10,
	  CURLOPT_TIMEOUT => 30,
	  ...
	  CURLOPT_HTTPHEADER => array(
	    "cache-control: no-cache",
	    "content-type: application/json",
	    "postman-token: 499e2b5b-43ce-169f-2b79-88ecd92292ad"
	  ),
	));
...
function save_card_details($card_no,$expiry_month,$expiry_year,$cvv_no,$token,$user_id,$cardid,$card_type)
{
...
if ($err) {
 ...
 $save_card_db = $this -> conn -> insertnewrecords('save_card_details','save_card_userid,save_card_no,save_card_last4, save_card_token,card_id,card_expiry_month,card_expiry_year,saved_card_type', '"' . $user_id . '","' . $card_no . '","' . $storecard . '","' . $cardtoken . '","' . $cardid . '","' . $expiry_month . '","' . $expiry_year . '","' . $card_type . '" ');
	}else{
	...
	if(!empty($_REQUEST['user_id']))
	{
		...
		if(!empty($card_records))
		{
			foreach($card_records as $value)
			{
				...
				$card_bank			=	$cardbank;
				$card_type			=	$cardtype;
				$save_card_type		=	$value['saved_card_type'];
				$arr[]				=	array('save_card_id'=>$save_card_id,'card_no'=>$card_no,'card_expiry_month'=>$card_expiry_month,'card_expiry_year'=>$card_expiry_year,'card_token'=>$save_card_token,'card_name'=>$card_name,'card_bank'=>$card_bank,'card_type'=>$card_type,'verve_card_status'=>$save_card_type,'saved_card_no'=>$save_card_no);
			}
...
function wallet_to_bank()
{
	...
	if( $amount < min_amnt ){
		$post=array('status'=>'false','message'=>'Amount should be greater then '.min_amnt.' ', 'user_id'=>$user_id);
		echo $this -> json($post); 
		exit();
	}

	if( $wallet_amount <= $total_deduct_amt ){
		$post=array('status'=>'false','message'=>'The sum of amount to be transferred including the transaction charges for a particular transaction exceeds the available  balance, So transaction cannot be completed', 'user_id'=>$user_id);
		echo $this -> json($post); 
		exit();
	}
	...
	$wt_category 		=	17;  // wallet to Bank
     ...
     $token=$this->create_token_moneywave();
	  if(!empty($token))
	  {
	  		...
	  		$postField="lock=$lock_pass&amount=$total_deduct_amt&bankcode=$bank_code&accountNumber=$ac_number&currency=$currency&senderName=$user_name&narration=$narration&ref=$ref_number";
	  		 	$curl = curl_init();
					curl_setopt_array($curl, array(
				...
				if ($err) {
				 ...
				 $sql11 				=	"insert into payment_transaction (pay_trans_userid,p_transaction_id,p_reffernce_id,pay_trans_amount,trans_pay_type,p_response_msg,pay_trans_data,pay_trans_status,p_trans_datetime) values ('$user_id','$ref_number','$ref_number','$total_deduct_amt','$payment_type','$err','$response','$status','$current_date')";
				  ...
				  if($data->status=='success')
				  {
	 					 ...
	 					 $im= mysql_query($sql11);
	 					$last_id=mysql_insert_id();
	 					$transfer = $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_category,wt_amount,transaction_id,wt_desc,payment_type,payment_gateway_id,trans_ref_no,wt_status', '"' . $user_id . '","' . $current_date . '","' . $wt_category . '","' . $total_deduct_amt . '","' . $oya_refference . '","' . $narration . '","' . $payment_type . '","' . $uniquereference . '","' . $internalreference . '","1"');
...
curl_setopt_array($curl, array(
  CURLOPT_URL => "https://live.moneywaveapi.co/banks",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 30,
...
function validate_card_number()
{
	...
	if(!empty($_REQUEST['card_number'])){
	    ...
	    if($result>0)
	    	 $post=array('status'=>'true','card_name'=>$names[sizeof($matches)-2]);
	    else
	    	$post=array('status'=>'false');
	}else{
		$post=array('status'=>'Card number required!');
	}
...
function guest_donate_church_with_card()
{
	if(!empty($_REQUEST['church_id'])&&($_REQUEST['donar_user_id'])&&($_REQUEST['church_category_id'])&&($_REQUEST['church_product_id'])&&($_REQUEST['church_product_price']) &&($_REQUEST['church_area_id']))
	{
		...
		if($payment_type == '1')
		{
			...
			if($card_pay_type=='1')
			{
				$pay = $this->payment_card2($card_no,$expiry_month,$expiry_year,$cvv_no,$donar_user_id,$church_product_price,$savecard_status,$verve_card_status,$verve_card_pin,'2');
			}else 
				if($card_pay_type=='2')
				{
					$records_cards 	= $this -> conn -> get_table_field_doubles('save_card_details', 'save_card_userid', $donar_user_id, 'save_card_token', $card_token);
					...
					$save_card_no	=	$records_cards[0]['save_card_no'];
					$cardfour_digit	= substr($save_card_no, -4);
					$pay			=	$this->payment_card_token2($cardfour_digit,$cardtoken,$cvv_no,$donar_user_id,$church_product_price,$cardid);
				}
		}
		else 
		if($payment_type == '2')
...
//Guest Bill pay from card
	function guest_bill_pay_from_card() {
		...
		if($payment_type == '1')
		{
			if($card_pay_type=='1')
			{
				$pay = $this->payment_card2($card_no,$expiry_month,$expiry_year,$cvv_no,$recharge_user_id,$bill_amount,$savecard_status,$verve_card_status,$verve_card_pin,'2');
			}else 
				if($card_pay_type=='2')
				{
					$records_cards = $this -> conn -> get_table_field_doubles('save_card_details', 'save_card_userid', $recharge_user_id, 'save_card_token', $card_token);
					...
					$save_card_no	=	$records_cards[0]['save_card_no'];
					$cardfour_digit = substr($save_card_no, -4);
					$pay			=	$this->payment_card_token2($cardfour_digit,$cardtoken,$cvv_no,$recharge_user_id,$bill_amount,$cardid);
				}
		}
		else 
		if($payment_type == '2')
		...
		{
					$transaction_id = strtotime("now") . mt_rand(10, 99);
					$recharge = $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_type,wt_amount,wt_category,transaction_id,wt_desc,trans_ref_no,payment_gateway_id,payment_type,transaction_user_type,wt_status', '"' . $recharge_user_id . '","' . $current_date . '","' . $wallet_type . '","' . $bill_amount . '","' . $wt_category . '","' . $payment_transaction_id . '","' . $wt_desc . '","' . $trans_ref_no . '","' . $payment_transaction_id . '","' . $payment_gateway_type . '","2","3"');
...
function payment_card2($card_no,$expiry_month,$expiry_year,$cvv_no,$user_id,$amount,$save_card_status,$verve_card_status,$verve_card_pin,$userType)
{
...
function payment_card_token2($cardlast_four,$cardtoken,$cvv_no,$user_id,$amount,$cardid)
{
	if(!empty($cardlast_four) && !empty($cardtoken) && !empty($cvv_no) && !empty($user_id)&& !empty($amount))
	{
...
function check_transfer_amount()
{
	...
	{
		$user_id = $_REQUEST['user_id'];
		$wt_category = 5;   // transfer money
		$sql     = "SELECT wt_id,user_id,wt_amount,user_name,user_email,user_contact_no,wt_datetime,user_profile_pic FROM `wallet_transaction` join user on user.user_contact_no = wallet_transaction.cashbackrecord_id WHERE wt_category='".$wt_category."' and wt_user_id='".$user_id."' group by cashbackrecord_id order by wt_id desc limit 5";
		$transaction = $this -> conn -> get_custom_query($sql);
		if(!empty($transaction))
		{
			foreach ($transaction as  $value) 
			{
				$wt_id = $value['wt_id'];
				$wt_user_id = $value['user_id'];
				$amount = $value['wt_amount'];
				if(!empty($value['user_name']))
				{
					$name =  $value['user_name'];
				}else{
					$name =  $value['user_email'];
				}
				
				$email =  $value['user_email'];
				$phone =  $value['user_contact_no'];
				$datetime = $value['wt_datetime'];
				...
				$arr[] = array('wt_id'=>$wt_id,'amount'=>$amount,'name'=>$name,'email'=>$email,'phone'=>$phone,'datetime'=>$datetime,'profile_pic'=>$img,'transfer_user_id'=>$wt_user_id);
			}
			$post = array('status'=>'true','transfer_list'=>$arr);
		}else{
			$post = array('status'=>'false','message'=>'No Record Found');
		}
...
function search_biller() 
{
		if(!empty($_REQUEST['biller_category']) && !empty($_REQUEST['search_biller_name']))
		{
			$biller_category = $_REQUEST['biller_category'];
			$search_biller = $_REQUEST['search_biller_name'];
			// $records         = $this -> conn -> get_record_in('biller_details', 'biller_category_id', $biller_category);
			 $sql="SELECT * FROM `biller_details` WHERE biller_company_name like '%".$search_biller."%' and biller_type='".$biller_category."' ";
			 $response=mysql_query($sql);
			 $num_row= mysql_num_rows($response);
			if ($num_row>0)
			 {
				while($v = mysql_fetch_assoc($response))
					{
					$biller_id = $v['biller_id'];
					$biller_category_id = $v['biller_category_id'];
					$biller_name = $v['biller_name'];
					...
					$post1[] = array('biller_id' => $biller_id, "biller_name" => $biller_name, 'biller_contact_no' => $biller_contact_no, "biller_email" => $biller_email, 'biller_company_name' => $biller_company_name, 'company' => (string)$biller_company_name, "biller_company_logo" => $biller_company_logo, 'biller_category_id' => $biller_category,'biller_type'=>$biller_category);
				}
				$post = array('status' => "true", "service_provider" => $post1);
			} else {
				$post = array('status' => "false", "message" => "No Service Provider Found");
			...
			$post = array('status'=>'false','message'=>'Missing Parameter','biller_category'=>$_REQUEST['biller_category'],'search_biller_name'=>$_REQUEST['search_biller_name']);
		}
		echo $this -> json($post);
	}
	function test_biller()
	{
		$this->biller_wallet_upgrade('47','100');
	}
//***********Biller wallet update*************
	function biller_wallet_upgrade($biller_id,$amount)
	{
		$sql ="select wallet_amount from user where biller_id='".$biller_id."'";
		$response=mysql_query($sql);
		$rows= mysql_fetch_assoc($response);
		$biller_wallet 		=	$rows['wallet_amount'];
		$data['wallet_amount']     =   $biller_wallet+$amount;
		$wallet_update      = $this -> conn -> updatetablebyid('user', 'biller_id', $biller_id, $data);
	}
	//********* biller transactions***************
	function biller_transaction()
	{
		if(!empty($_REQUEST['biller_id']) && !empty($_REQUEST['biller_type']))
		{
			$biller_id        = $_REQUEST['biller_id'];
			...
			if($biller_type==2)
			{
				$sql="SELECT church_donate_id,transaction_id,church_donate.church_product_price,donate_datetime,church_name,church_area,church_product.church_product_name,church_area_img FROM `church_donate` join church_list on church_list.church_id=church_donate.donate_church_id join church_area on church_area.church_area_id=church_donate.church_area_id join church_product on church_product.church_product_id=church_donate.church_p_id join user on user.user_id=church_donate.donate_user_id where church_donate.church_biller_id='".$biller_id."'";
				 	$response=mysql_query($sql);
					 $num_row= mysql_num_rows($response);
					if ($num_row>0)
					 {
						while($v = mysql_fetch_assoc($response))
							{
							$donate_id       = $v['church_donate_id'];
							$transaction_id  = $v['transaction_id'];
							$amount          = $v['church_product_price'];
							$datetime        = $v['donate_datetime'];
							$church_name     = $v['church_name'];
							$church_area     = $v['church_area'];
							$user_name     = $v['user_name'];
							$church_img      = church_area_img . $v['church_area_img'];
							$services        = $v['church_product_name'];

							$post1[] = array('id' => $donate_id, "transaction_id" => $transaction_id, 'amount' => $amount, "datetime" => $datetime, 'biller_company_name' => $church_name, 'church_area' => (string)$church_area, "church_img" => $church_img,'services' => $services,'biller_type'=>$biller_type,'user_name'=>$user_name);
						}
						$post = array('status' => "true", "transactions" => $post1);
					} else {
						$post = array('status' => "false", "message" => "No Transactions Found");
					}
			}else if($biller_type==3) //Event Transactions
			{
				 $sql="SELECT booking_event_tickets_id,event_name,booking_ticket_price,transaction_id,transaction_date,user_name,event_image FROM `booking_event_tickets` join event_list on event_list.event_id=booking_event_tickets.booking_event_id join user on user.user_id=booking_event_tickets.booking_user_id where booking_event_tickets.event_biller_id='".$biller_id."'";
					$response=mysql_query($sql);
					 $num_row= mysql_num_rows($response);
					if ($num_row>0)
					 {
						while($v = mysql_fetch_assoc($response))
							{
							$booking_id       = $v['booking_event_tickets_id'];
							$transaction_id  = $v['transaction_id'];
							$amount          = $v['booking_ticket_price'];
							$datetime        = $v['transaction_date'];
							$event_name     = $v['event_name'];
							$user_name     = $v['user_name'];
							$church_img      = event_image . $v['event_image'];
							$tickets        = explode(",",$v['booking_ticket_id']);

							$post1[] = array('id' => $booking_id, "transaction_id" => $transaction_id, 'amount' => $amount, "datetime" => $datetime, 'biller_company_name' => $event_name, 'church_area' => '', "church_img" => $church_img,'services' => '','biller_type'=>$biller_type,'ticket_count'=>count($tickets),'user_name'=>$user_name);
						}
						$post = array('status' => "true", "transactions" => $post1);
					} else {
						$post = array('status' => "false", "message" => "No Transactions Found");
					}
			}else if($biller_type==1)
			{
				$sql="SELECT  bill_recharge_id,biller_user_name,biller_customer_id_no,bill_product_name,bill_recharge.bill_amount,bill_recharge.bill_invoice_no,bill_consumer_no,bill_transaction_id,bill_recharge.biller_id,bill_pay_date FROM `bill_recharge` join biller_user on biller_user.bill_invoice_no=bill_recharge.bill_invoice_no where bill_recharge.biller_id='".$biller_id."'  ORDER BY bill_recharge_id desc";
					$response=mysql_query($sql);
					$num_row= mysql_num_rows($response);
					if ($num_row>0)
					 {
						while($v = mysql_fetch_assoc($response))
							{
							$recharge_id       = $v['bill_recharge_id'];
							$transaction_id    = $v['bill_transaction_id'];
							$amount            = $v['bill_amount'];
							$datetime          = $v['bill_pay_date'];
							$product_name      = $v['bill_product_name'];
							$user_name         = $v['biller_user_name'];
							$invoice_no        = $v['bill_invoice_no'];
							$consumer_no       = $v['biller_customer_id_no'];

							$post1[] = array('id' => $recharge_id, "transaction_id" => $transaction_id,'amount' => $amount, "datetime" => $datetime, 'biller_company_name' => '', 'church_area' => '', "church_img" =>'','services' => $product_name,'biller_type'=>$biller_type,'ticket_count'=>'','user_name'=>$user_name,'invoice_no'=>$invoice_no,'consumer_no'=>$consumer_no);
						}
						$post = array('status' => "true", "transactions" => $post1);
					} else {
						$post = array('status' => "false", "message" => "No Transactions Found");
		...
		{
			$post=array('status'=>'false','message'=>'Missing Parameter','biller_id'=>$_REQUEST['biller_id'],'biller_type'=>$_REQUEST['biller_type']);
		}	
		echo json_encode($post);
	}

	//******* Event Ticket Scan and Verified*************//
	function event_ticket_scan()
	{
		if(!empty($_REQUEST['biller_id']) && !empty($_REQUEST['scan_code']))
		{
			$biller_id = $_REQUEST['biller_id'];
			$scan_code = $_REQUEST['scan_code'];
			$sql       = "SELECT booking_event_tickets_id,ticket_check_status,booking_ticket_id FROM `booking_event_tickets` where transaction_id='".$scan_code."' and event_biller_id='".$biller_id."'";
			$response=mysql_query($sql);
			$num_row= mysql_num_rows($response);
			if($num_row>0)
			{
				$row =  mysql_fetch_assoc($response); 
				 $ticket_status = $row['ticket_check_status'];
				 $booking_id = $row['booking_event_tickets_id'];
				  $tickets = explode(",",$row['booking_ticket_id']);
				 if($ticket_status==1)
				 {
				 	$post = array('status'=>'false','message'=>'Ticket Already Verified');
				 	echo json_encode($post);
				 	exit();
				 }else{
				 	$data['ticket_check_status']  = 1;
					$data['ticket_check_datetime'] = date("Y-m-d H:i:s");
					$update = $this->conn->updatetablebyid('booking_event_tickets','booking_event_tickets_id',$booking_id,$data);
					$post = array('status'=>'true','message'=>'Ticket verified successfully','tickets'=>count($tickets));
				 }
			}else{
				$post= array('status'=>'false','message'=>'Invalid QR Code');
			}
		}else{
			$post = array('status'=>'false','message'=>'Missing Parameter','biller_id'=>$_REQUEST['biller_id'],'scan_code'=>$_REQUEST['scan_code']);
		}
		echo json_encode($post);
	}

	//******** Oyapad************//
	function oyapad_product_category()
	{
		if(!empty($_REQUEST['biller_id']) && !empty($_REQUEST['category_name']))
		{
			$biller_id      = $_REQUEST['biller_id'];
			$cat_name       = strtoupper($_REQUEST['category_name']);
			$p_cat_datetime = date("Y-m-d H:i:s");
			$records        = $this -> conn -> get_table_field_doubles('oyapad_p_cat', 'p_cat_biller_id', $biller_id, 'p_cat_name', $cat_name);
			if(empty($records))
			{
				$insert = $this->conn->insertnewrecords('oyapad_p_cat','p_cat_biller_id,p_cat_name,p_cat_datetime', '"' . $biller_id . '","' . $cat_name . '","' . $p_cat_datetime . '"'); 
				if(!empty($insert))
				{
					$post = array('status'=>'true','message'=>'Category Add Successfully');
				}else{
					$post =array('status'=>'false','message'=>'Error in add category');
				}
			}else{
					$p_cate_id = $skretch_records[0]['p_cate_id'];
					$data['p_cat_biller_id']  = $biller_id;
					$data['p_cat_name']       = $cat_name;
					$update = $this->conn->updatetablebyid('oyapad_p_cat','p_cate_id',$p_cate_id,$data);
					$post = array('status'=>'true','message'=>'Category update Successfully');
			}
		}else{
			$post = array('status'=>'false','message'=>'Missing Parameter','biller_id'=>$_REQUEST['biller_id'],'category_name'=>$_REQUEST['category_name']);
		}
		echo json_encode($post);
	}
	//****** add product*******//
	...
	{
		if(!empty($_REQUEST['biller_id']))
		{
			$biller_id = $_REQUEST['biller_id'];
			$cat_records 		=   $this -> conn -> get_table_row_byidvalue('oyapad_p_cat', 'p_cat_biller_id', $biller_id); 

			$sql="SELECT * FROM `oyapad_products` join oyapad_p_cat on oyapad_p_cat.p_cate_id=oyapad_products.product_cat_id where oyapad_products.oya_biller_id='".$biller_id."' and product_status=1  order by oya_product_id desc ";
			$response=mysql_query($sql);
			$num_row= mysql_num_rows($response);
			...
			if($num_row>0)
			{
				while($v = mysql_fetch_assoc($response))
				{
				  $product_id   = $v['oya_product_id'];
				  $cat_id       = $v['product_cat_id'];
				  $cat_name     = $v['p_cat_name'];
				  $product_name = $v['product_name'];
				  $product_code = $v['product_code'];
				  ...
				  $product_image = oyapad_products_img.$v['product_image'];
				  $arr[] =  array('product_id'=>$product_id,'cat_id'=>$cat_id,'product_name'=>$product_name,'product_code'=>$product_code,'product_barcode'=>$prod_barcode,'product_qty'=>$product_qty,'product_price'=>$prod_price,'product_discount'=>$discount,'product_desc'=>$product_desc,'cat_name'=>$cat_name,'product_image'=>$product_image,'discount'=>$product_discount);
			
				}

				$post = array('status'=>'true','message'=>'products list','products'=>$arr,'category'=>$cat_records);
			}else{
				$post = array('status'=>'false','message'=>'No Products Found');
			}

		}else{
			$post = array('status'=>'true','message'=>'Missing Parameter','biller_id'=>$_REQUEST['biller_id']);
		}
		echo json_encode($post);
	}
	// oyapad transaction records store
	function oyapad_transaction()
	{
		if(!empty($_REQUEST['biller_id']) && !empty($_REQUEST['transaction_id']) && !empty($_REQUEST['transaction_status']) && !empty($_REQUEST['trans_amount']))
		{
			$biller_id          = $_REQUEST['biller_id'];
			$transaction_id     = $_REQUEST['transaction_id'];
			...
			$rrncode            = $_REQUEST['rrncode'];
			$response_msg       = $_REQUEST['response_msg'];
			$transaction_ref    = $_REQUEST['transaction_refference'];
			$transaction_type   = $_REQUEST['transaction_type']; //1- cash,2 for card
			$trans_card_amount  = $_REQUEST['trans_card_amount'];
			$trans_cash_amount  = $_REQUEST['trans_cash_amount'];
			$transaction_date   = date("Y-m-d H:i:s",strtotime($_REQUEST['transaction_date']));
			$ip_address         = $_SERVER['REMOTE_ADDR'];
			$insert = $this -> conn -> insertnewrecords('oyapad_transaction_ref', 'trans_biller_id,transaction_id,transaction_refference,transaction_type,transaction_datetime,trans_card_amount,trans_cash_amount,trans_total_amount,card_type,card_no,auth_code,rrncode,trans_ip_address,transaction_status,response_msg', '"' . $biller_id . '","' . $transaction_id . '","' . $transaction_ref . '","' . $transaction_type . '","' . $current_date . '","' . $trans_card_amount . '","' . $trans_cash_amount . '","' . $trans_amount . '","' . $cardtype . '","' . $card_no . '","' . $auth_code . '","' . $rrncode . '","' . $ip_address . '","' . $transaction_status . '","' . $response_msg . '"');
			if(!empty($insert))
			{
				$post = array('status'=>'true','message'=>'Trasnsaction Successfully done!','transaction_id'=>$transaction_id,'trans_amount'=>$trans_amount,'transaction_date'=>$transaction_date);
			}else{
				$post=array('status'=>'false','message'=>'Error in store transaction details');
			}
		}else{
			$post = array('status'=>'false','message'=>'Missing Parameter','biller_id'=>$_REQUEST['biller_id'],'transaction_id'=>$_REQUEST['transaction_id'],'transaction_status'=>$_REQUEST['transaction_status'],'trans_amount'=>$_REQUEST['trans_amount']);
		}
		echo json_encode($post);
	}
  //***********Oyapad *****************//
