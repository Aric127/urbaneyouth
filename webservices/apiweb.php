	...
	*/
	...
	//	$pay	=	$this->payment_card($card_no,$expiry_month,$expiry_year,$cvv_no,$user_id,$user_amount);
		...
		if($transactionStatus == 'fail' OR $transactionStatus == 'error'){
			...
			$post = array('status' => "false", "message" => $responseMessage,'rc' => $rc,'transactionStatus'=>$transactionStatus,'id'=>$id,'ref'=>$ref);
			echo $this -> json($post);
			exit();
...
//	$pay	=	$this->payment_card($card_no,$expiry_month,$expiry_year,$cvv_no,$recharge_user_id,$refund_amount);
	...
	//	print_r($status); exit;
		...
		if($transactionStatus == 'fail' OR $transactionStatus == 'error'){
			...
			$post = array('status' => "false", "message" => $responseMessage,'rc' => $rc,'transactionStatus'=>$transactionStatus,'id'=>$id,'ref'=>$ref,'transaction_via'=>$transaction_via);
			echo $this -> json($post);
			exit();
		...
		$post = array('status' => "false", "message" => 'Recharge successfully',  'transaction_id' => $payment_transaction_id, 'transaction_date' => date('F-d-Y h:i A',strtotime($current_date)),'transaction_ref'=>$trans_ref_no,'transaction_via'=>$transaction_via, 'transactionStatus' => $transactionStatus, 'rc' => $rc, 'responseMessage' => $query);

 echo $this -> json($post); exit;

		//$arr = json_decode($query['resp']);
		//$responsecode    =  $arr->responsecode; 
		//$responsemessage =  $arr->responsecode; 
		//$responsehtml    =  $arr->responsecode; 

		//echo 'stop recharge'; exit;
  ...
  if($status == 'success')
  {
	...
	if($save_card_status=='1')
		...
		return array( 'status' => $status, 'trans_id' => $trans_id, 'ref_id' => $ref_id, 'last_id' => $last_id, 'authurl' => $authurl, 'response_msg' => $response_msg, 'chargeMethod' => $chargeMethod );
  		...
  		return array('status' => $data->status, 'msg' => $msg);
...
if ($err) {
  ...
  if($status == 'success')
  {
	...
	// $insert = $this -> conn -> insertnewrecords('payment_transaction','pay_trans_userid,p_transaction_id,p_reffernce_id, pay_trans_amount,trans_pay_type,p_response_msg,pay_trans_data,pay_trans_status,p_trans_datetime', '"' . $user_id . '","' . $trans_id . '","' . $ref_id . '","' . $trans_amount . '","' . $payment_type . '","' . $respoonse_msg . '", "'".$data1212."'" ,"'.$status.'","' . $current_date . '"');
        ...
        return array('status' => $status, 'trans_id' => $trans_id, 'ref_id' => $ref_id, 'last_id' => $last_id, 'authurl' => $authurl);
	...
	// $trans_id		=	$record->id;
	 ...
	 $im= mysql_query($sql);
  	...
  	//return  $data->status."/".$msg;
			...
			return array('status' => $data->status, 'msg' => "Error in create token");
			exit();
	}
}
...
function chargeAuth()
{
	...
	curl_setopt_array($curl, array(
	  CURLOPT_URL => "https://moneywave.herokuapp.com/v1/transfer/charge/auth/card",
	  CURLOPT_RETURNTRANSFER => true,
	  CURLOPT_ENCODING => "",
	  CURLOPT_MAXREDIRS => 10,
	  CURLOPT_TIMEOUT => 30,
...
function AcoountOtpVerify()
{
	...
	$curl = curl_init();
	 ...
	 curl_setopt_array($curl, array(
	  CURLOPT_URL => "https://moneywave.herokuapp.com/v1/transfer/charge/auth/account",
	  CURLOPT_RETURNTRANSFER => true,
	  CURLOPT_ENCODING => "",
	  CURLOPT_MAXREDIRS => 10,
	  CURLOPT_TIMEOUT => 30,
