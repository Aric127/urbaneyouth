...
class API extends REST {
	...
	function social_login() {
		...
		if (!empty($user_email) && !empty($user_social_id) && !empty($login_type)) {
			...
			if (!empty($user_id)) {
				...
				$insert = $this -> conn -> insertnewrecords('user', 'user_name,user_email,user_social_id,user_login_type,user_created_date,user_profile_pic,user_device_type,user_device_token,user_refferal_code', '"' . $user_name . '","' . $user_email . '","' . $user_social_id . '","' . $login_type . '","' . $current_date . '","' . $profile_pic . '","' . $user_device_type . '","' . $user_device_token . '","' . $reffer_code . '"');
				if ($insert) {
					$wallet_amount = 0;

					$post = array("status" => "true", "message" => "Login Successfully", 'user_id' => $insert, 'login_type' => $login_type, 'user_name' => $user_name, 'wallet_amount' => $wallet_amount, 'profile_pic' => $profile_pic, 'user_email' => $user_email, 'user_contact_no' => '', 'verify_status' => '2', 'user_pin_status' => '2', 'frnd_reffer_code' => $frnd_reffer_code);
				}
			}
		} else {
	...
	function forget_password() {
		...
		if (!empty($login_id)) {
			...
			if (!empty($records_user)) {
				...
				sendElasticEmail($user_email, $mail_msg,"OyaCharge", $message, "care@oyacharge.com", "OyaCharge");
					...
					foreach ($transaction as $v) {
						...
						if ($transaction_status == '1') {
							if ($transaction_type == '1') {
								...
								$transaction_desc = "Amount is Refund is successfully";
							} else if ($transaction_type == '4') {
								$transaction_desc = "Cashback is Recieved on Recharge of " . $cashback_record;
							} else if ($transaction_type == '5') {
								$transaction_desc = $transaction_desc;
	...
	function add_contact() {
		//$path = 'http://'.$_SERVER['HTTP_HOST'].'/Recharge/uploads/contacts/';
		$user_id = $_POST['user_id'];
			...
			if ($wallet_amount >= $recharge_amount) {
				...
				if ($recharge) {
					...
					if ($reffer_status == '2') {
						...
						$wt_desc = "Refferal amount add in your wallet using by " . substr($frnd_number, 4);
					...
					$this -> recharge_message($mobile_number, $recharge_amount, $transaction_id);
			 ...
			 $walletrecharge = $this -> conn -> insertnewrecords('recharge','recharge_transaction_id,recharge_user_id, rechage_category,operator_id,rechage_type,recharge_number,recharge_amount,recharge_date,recharge_status','"'.$transaction_id . '","' . $recharge_user_id . '","' . $recharge_category_id . '","' . $operator_id . '","' . $recharge_type_id . '","' .$recharge_number. '","' . $recharge_amount . '","' . $current_date . '","3"');
			 $post = array('status' => "false","message" => "Recharge failed", "recharge_id" => $recharge,'recharge_number'=>$rec_number,'recharge_amount'=>$recharge_amount,'wallet_used_amount'=>$wallet_used_amount,'card_used_amount'=>$card_deduct_amount,'wallet_amount'=>$wallet_amount,'recharge_date'=>$current_date,'transaction_id'=>$transaction_id);
...
<table cellpadding="0" cellspacing="0" width="600" style="background:#fff; border:1px solid #cbcbcb; margin:0 auto; font-family:Arial, Helvetica, sans-serif; font-size:12px;">
	...
	function recharge_from_wallet_with_card() {
		...
		if (!empty($_REQUEST['trans_id'])) {
			$payment_transaction_id = $_REQUEST['trans_id'];
		} else {
		...
		if ($operator_code == 'MTN') {
			...
			//}
			if ($recharge_response == '1') {
				$recharge = $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_type,wt_amount,wt_category,transaction_id,wt_desc,user_wallet_rec_amount,user_card_card_rec_amount,payment_gateway_id', '"' . $recharge_user_id . '","' . $current_date . '","' . $wallet_type . '","' . $recharge_amount . '","' . $wt_category . '","' . $transaction_id . '","' . $wt_desc . '","' . $wallet_amount . '","' . $card_deduct_amount . '","' . $payment_transaction_id . '"');

				if ($recharge) {
					$walletrecharge = $this -> conn -> insertnewrecords('recharge', 'recharge_transaction_id,recharge_user_id, rechage_category,operator_id,rechage_type,recharge_number,recharge_amount,recharge_date,recharge_status,payment_gateway_id', '"' . $transaction_id . '","' . $recharge_user_id . '","' . $recharge_category_id . '","' . $operator_id . '","' . $recharge_type_id . '","' . $recharge_number . '","' . $recharge_amount . '","' . $current_date . '","' . $recharge_status . '","' . $payment_transaction_id . '"');
			...
			$records_coupon = $this -> conn -> get_table_row_byidvalue('offer_coupon', 'coupon_id', $coupon_id);
					...
					$update_toekn = $this -> conn -> updatetablebyid('admin', 'admin_id', 1, $data_admin);
					$this -> recharge_message($mobile_number, $recharge_amount, $transaction_id);
					$post = array("status" => "true", 'message' => "Recharge Successfully", "recharge_id" => $recharge, 'recharge_number' => $rec_number, 'recharge_amount' => $recharge_amount, 'wallet_used_amount' => $wallet_used_amount, 'card_used_amount' => $card_deduct_amount, 'wallet_amount' => $wallet_amount, 'recharge_date' => $current_date, 'transaction_id' => $transaction_id, 'transaction_id' => $payment_transaction_id);
					$this -> offer_mail($recharge_user_id);
				} else {
					$post = array('status' => "false", "message" => "Recharge failed");
				}
				...
				$recharge = $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_type,wt_amount,wt_category,transaction_id,wt_desc,wt_status,payment_gateway_id', '"' . $recharge_user_id . '","' . $current_date . '","' . $wallet_type . '","' . $recharge_amount . '","' . $wt_category . '","' . $transaction_id . '","' . $wt_desc . '","3","' . $payment_transaction_id . '"');
				...
				$post = array('status' => "false", "message" => "Recharge failed", "recharge_id" => $recharge, 'recharge_number' => $rec_number, 'recharge_amount' => $recharge_amount, 'wallet_used_amount' => $wallet_used_amount, 'card_used_amount' => $card_deduct_amount, 'wallet_amount' => $wallet_amount, 'recharge_date' => $current_date, 'transaction_id' => $payment_transaction_id);
			}
	...
	function recharge_from_card() {
		...
		if ($operator_code == 'MTN') {
			...
			//}
			if ($recharge_response == '1') {
				$recharge = $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_type,wt_amount,wt_category,transaction_id,wt_desc,payment_gateway_id', '"' . $recharge_user_id . '","' . $current_date . '","' . $wallet_type . '","' . $recharge_amount . '","' . $wt_category . '","' . $transaction_id . '","' . $wt_desc . '","' . $payment_transaction_id . '"');
				...
				if ($recharge) {
					...
					$update_toekn = $this -> conn -> updatetablebyid('admin', 'admin_id', 1, $data_admin);
					$this -> recharge_message($mobile_number, $recharge_amount, $transaction_id);
					$post = array("status" => "true", 'message' => "Recharge Successfully", "recharge_id" => $recharge, 'recharge_number' => $rec_number, 'recharge_amount' => $recharge_amount, 'wallet_amount' => $wallet_amount, 'recharge_date' => $current_date, 'transaction_id' => $transaction_id, 'MTN_recharge_status' => $recharge_responsem, 'transaction_id' => $payment_transaction_id);
					$this -> offer_mail($recharge_user_id);
				} else {
					$post = array('status' => "false", "message" => "Recharge failed");
				}
				...
				$transaction_id = strtotime("now") . mt_rand(10000000, 99999999);
				$refund_desc = "amount is refund for failed transaction of recharge";
				$refund_record = $this -> conn -> insertnewrecords('wallet_transaction', 'wt_user_id, wt_datetime,wt_type,wt_amount,wt_category,transaction_id,wt_desc,wt_status', '"' . $recharge_user_id . '","' . $current_date . '","' . $wallet_type . '","' . $refund_amount . '","3","' . $transaction_id . '","' . $refund_desc . '","1"');

				$data_refund['refund_status'] = 1;
				$data_refund['cashbackrecord_id'] = $transaction_id;
				$update_wallet = $this -> conn -> updatetablebyid('wallet_transaction', 'transaction_id', $trans_id, $data_refund);

				$post = array('status' => "false", "message" => "Recharge failed", "recharge_id" => $recharge, 'recharge_number' => $rec_number, 'recharge_amount' => $recharge_amount, 'wallet_amount' => $wallet_amount, 'recharge_date' => $current_date, 'transaction_id' => $payment_transaction_id);
			}
	...
	function transfer_money() {
		...
		if (!empty($mobile)) {
			...
			if (!empty($user_records)) {
				...
				$contact_number_main = substr($user_records['0']['user_contact_no'], 4);
				// main user mobile number
				if ($main_wallet >= $amount) {
					$records = $this -> conn -> get_table_row_byidvalue('user', 'user_contact_no', $mobile);
					if (!empty($records)) {
						$contact_number_frnd = substr($records['0']['user_contact_no'], 4);
						// frnd mobile number
	...
	function pdf() {
		...
		// $pdf->SignUp($data);
		// print_r($pdf->Output());
		// $pdf->Output('/var/www/Recharge/uploads/invoice/'.'Bill'.'.pdf','F');
	}
		...
		if (!empty($biller_id)) {
			...
			if ($records) {
				foreach ($records as $v) {
					...
					$post1[] = array('biller_id' => $biller_id, "product_id" => $product_id, 'product_name' => $product_name, 'product_code' => $product_code);
				}
	...
	function create_invoice() {
		...
		$bill_amount = $_REQUEST['bill_amount'];
		$bill_product_id = $_REQUEST['bill_product_id'];
		for ($i = 0; $i < count($bill_product_id); $i++) {

			$biller_product = mysql_query("SELECT * FROM `biller_produt` WHERE find_in_set(product_id, '" . $bill_product_id . "')");

			//$where12=array('find_in_set(product_id, "'.$bill_product_id[$i].'") '=>NULL);
			//$biller_product =  $this -> conn -> get_table_row_byidvalue('biller_produt','find_in_set(product_id, "'.$bill_product_id[$i].'")');
			while ($row = mysql_fetch_array($biller_product)) {
				$name[] = $row['product_name'];
			}

		}
		$bill_product_name = implode(",", $name);
	...
	function mobile_recharge_api($operator_id, $mobile, $amount) {
     ...
     // corncone api for recharge==================//////////////////==========
		else if($operator_code == 'AQA' || $operator_code == 'AWA' || $operator_code == 'AQC' || $operator_code == 'ANA'||  $operator_code == 'APA'	)
		{
			$phone=$mobile;
			$id=rand('1000','9999');
	...
	function recharge_message($mobile_no, $amount, $transaction_id) {

		$mobile = "0" . $mobile_no;
		$msg = "Recharge Successfully done from OyaCharge of  : " . $mobile_no . " of recharge of " . $amount . " " . Niera . ". Transaction id  " . $transaction_id;
